...
def _xcore1600_sources():
    ...
    Generated by `scripts/generate_instantiations.py` and listed explicitly, as
    upstream does: the set is what gets compiled, so it should be visible
    rather than globbed.
    """
...
def build_for_maca():
    ...
    **Every architecture is built from `csrc/xcore1000/`.**  That tree is the
    ...
    (CLAUDE.md, "Can a C600U run the C500 kernel").  The port stays in the tree,
    compiling, as the reserved implementation; reaching it is an edit to the
    one `sources =` line below rather than an environment variable -- a switch
    that could put the broken kernel back is a switch that can be left on.
    ...
    `CUCC_TARGETS` (default `native`), same variable and meaning as the host
    repository's `build.sh`; `-offload-arch` is passed per extension, so a build
    targets exactly one while staying independent of which device is visible.
    ...
    include_dirs = [
        ...
        # which the ported kernels include.  Vendored under `csrc/3rdparty`;
        # upstream's layout differs, so it is spelled out rather than guessed.
    ...
    for target in targets:
        ...
        # `_xcore1600_sources()` is the reserved other answer; swapping it in is
        # the whole of "build the port".
