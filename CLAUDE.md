...
Public entry point is `deep_select.topk` (`deep_select/interface.py`). **Its signature is upstream's, parameter for parameter** — the same 14 positional-or-keyword arguments in the same order with the same defaults (`input … abort_when_nan_found`) — plus exactly two added keywords, `backend: Optional[str] = None` and `check_nan: bool = True`, **both appended after upstream's last argument**. That is checkable rather than asserted: `git diff upstream/main HEAD -- deep_select/interface.py` shows the `def topk(` block unchanged apart from the added parameters, and `upstream/main` is a direct ancestor of this branch. An upstream call site works here unchanged, including one that passes every argument positionally — which is the whole reason the added keywords go *after* `abort_when_nan_found` rather than beside the argument they belong with: `check_nan` is the NaN check's switch and reads naturally next to it, but inserting it there would shift `backend` under a caller that passed its 15 arguments positionally, silently rebinding a backend name to a bool. Upstream exports `topk` and `get_stride_requirement`; this tree adds `UnsupportedByBackend`, `topk_torch` and `topk_deep_gemm`.
...
**`check_nan=False` is a different switch from `abort_when_nan_found`, and the two are not interchangeable.** The first decides whether the row is scanned; the second what happens once one is found. With `check_nan=False` nothing is ever found, so `abort_when_nan_found` is inert, no sentinel is written, and **a NaN is ranked as a value like any other** — under the key encode it lands at one end of the row. That is the meaning of turning the switch off: the caller asserts its input is NaN-free and the operator takes it at its word. It is not a faster way to get the `abort_when_nan_found=False` behaviour.

The scan is a full extra pass over the row, and on the chunked paths it is a *separate grid-wide kernel* (`nan_scan_kernel`, launched wide enough to cover every chunk), so turning it off is a real saving on a known-clean input: measured on C500, **22–32% of the five official fp32 cells** (`7243.7 → 5618.1 µs` at `b4096-v129280`, `98.2 → 66.7` at `b6`). §8.4 of `docs/C500-radix-perf-ledger.zh.md` carries the table and the gate; §8.1/§8.2 there are where the same scan was priced against `deep_gemm`'s `11.8 ms` `torch.isnan` check. The switch is a **contract** flag, not a tuning knob: nothing in this tree sets it, the suites run with the default, and a caller that wants the kernel's speed on verified-clean input is the only caller that should pass it.

**It is a separate switch from `abort_when_nan_found`, and both are measured — do not collapse them.** Also measured while landing it, and worth knowing before writing any check that compares two calls elementwise: **the slot order inside an unordered fp32 row is not deterministic**, five identical calls give five orderings, `check_nan` on or off alike. The selected *set* is identical and `min(selected) >= max(unselected)` holds, so the contract is met — but `sorted=False` promises no order, and an elementwise-equality assertion between two runs will fail here. Use the contract, or ask for `sorted_index`.
